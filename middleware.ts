...
export async function middleware(req: NextRequest) {
    ...
    if (supabaseUrl && serviceKey) {
      ...
      if (res.ok) {
        ...
        if (rows.length > 0) {
          ...
          let rlResult: Awaited<ReturnType<typeof checkRateLimit>> | null =
            null;
          if (pathname.startsWith("/api/v1/")) {
            ...
            if (!rlResult.allowed) {
              ...
              return NextResponse.json(
                {
                  error: "Rate limit exceeded. See Retry-After header.",
                  code: "RATE_LIMIT",
                },
                {
                  status: 429,
          ...
          if (wsBillingStatus === "suspended_for_nonpayment") {
            return NextResponse.json(
              {
                error: "Workspace suspended for non-payment. Contact support.",
  ...
  if (!hasSessionCookie) {
    return buildLoginRedirect(
      req,
      pathname,
      `no-session-cookie|cookies:${allCookies.map((c) => c.name).join(",")}`,
    );
  }
  ...
  if (!user) {
    return buildLoginRedirect(
      req,
      pathname,
      `getUser-null|error:${getUserError?.message ?? "none"}|cookies:${allCookies.map((c) => c.name).join(",")}`,
    );
  }
  ...
  if (impToken && supabaseUrl && process.env["SUPABASE_SERVICE_ROLE_KEY"]) {
    ...
    try {
      ...
      if (impRes.ok) {
        const rows = (await impRes.json()) as {
          id: string;
        ...
        if (rows.length > 0 && new Date(rows[0]!.expires_at) > new Date()) {
          supabaseResponse.headers.set(
            "x-impersonation-workspace-id",
            rows[0]!.target_workspace_id,
          );
        }
      }
    } catch {
      /* non-fatal — impersonation simply won't activate */
    }
  }
