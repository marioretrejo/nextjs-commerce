...
export default function InboundRoutingPage({
  params,
}: {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (routingRes.ok) {
        const d = (await routingRes.json()) as {
          number: string;
          ...
          <Card>
            ...
            <CardContent>
              <Select
                ...
                onValueChange={(v) =>
                  setConfig((c) => ({ ...c, default_agent_id: v || null }))
                }
              >
                <SelectTrigger>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <div>
                ...
                <CardDescription>
                  Rules are evaluated in priority order, top first.
                </CardDescription>
              </div>
            ...
            <CardContent className="space-y-4">
              {config.rules.length === 0 ? (
                <p className="text-sm text-[#6b6b6b] text-center py-4">
                  No rules yet. Add a rule above.
                </p>
              ) : (
                config.rules.map((rule, idx) => (
                  <div
                    key={rule.id}
                    className="rounded-lg border border-[#e0e0e0] p-4 space-y-3"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="space-y-1.5">
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="time_of_day">
                              Time of Day
                            </SelectItem>
                            <SelectItem value="caller_id_pattern">
                              Caller ID Pattern
                            </SelectItem>
                          </SelectContent>
                        </Select>
                    ...
                    {(rule.condition === "time_of_day" ||
                      rule.condition === "caller_id_pattern") && (
                      <div className="space-y-1.5">
