...
function dispositionBadge(disposition: CallDisposition | null) {
  ...
  return (
    <Badge className={`${cfg.className} whitespace-nowrap text-xs`}>
      {cfg.label}
    </Badge>
  );
}
...
export default function CallsPage() {
  ...
  const [dispositionFilter, setDispositionFilter] =
    useState<DispositionFilter>("all");
  ...
  const fetchCalls = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      if (dispositionFilter !== "all")
        all = all.filter((c) => c.disposition === dispositionFilter);
      setCalls(all);
    }
    setLoading(false);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        <select
          ...
          onChange={(e) =>
            setDispositionFilter(e.target.value as DispositionFilter)
          }
          className="h-9 rounded-md border border-[#e0e0e0] bg-white px-3 text-sm text-[#0a0a0a] focus:outline-none focus:ring-1 focus:ring-[#0a0a0a]"
        >
          {dispositions.map((d) => (
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-[#e0e0e0]">
              {calls.map((call) => (
                <div
                  key={call.id}
                  className="px-5 py-4 hover:bg-[#f5f5f5] transition-colors"
                >
