    ...
    return raw
      ...
      .map((line) => {
        if (line.startsWith("Agent:"))
          return { role: "agent", text: line.slice(6).trim() };
        if (line.startsWith("User:"))
          return { role: "user", text: line.slice(5).trim() };
  ...
  async function listenIn(call: LiveCall) {
    ...
    try {
      const res = await fetch(
        `/api/calls/live/${encodeURIComponent(call.retell_call_id)}/token`,
      );
      if (!res.ok)
      ...
      toast.success(
        `Listening to call with ${call.contact_name ?? "unknown contact"}`,
      );
    } catch (e) {
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              ...
              <p className="text-xs text-[#6b6b6b] mt-1">
                Calls in progress will appear here
              </p>
            </div>
          ) : (
            calls.map((call) => (
              ...
              >
                ...
                <p className="text-xs text-[#6b6b6b] font-mono mb-1">
                  {call.contact_phone ?? "—"}
                </p>
        ...
        {!selected ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-[#6b6b6b]">
              Select an active call to view real-time details
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="p-5 border-b border-[#e0e0e0] flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[#6b6b6b]">
                  {selected.contact_phone} · {selected.agent?.name}
                </p>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div className="text-right">
                  ...
                  <p
                    className={`text-sm font-medium capitalize ${sentimentColor(selected.sentiment)}`}
                  >
                    {selected.sentiment ?? "—"}
                  </p>
                </div>
                  ...
                  >
                    <div
                      className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                        line.role === "agent"
                          ...
                          : "bg-[#f5f5f5] text-[#0a0a0a] border border-[#e0e0e0]"
                      }`}
                    >
                      {line.role === "agent" ? "A" : "U"}
                    </div>
                    <Card className="max-w-md">
                      <CardContent className="p-3">
                        <p className="text-xs font-medium text-[#6b6b6b] mb-0.5 capitalize">
                          {line.role}
                        </p>
