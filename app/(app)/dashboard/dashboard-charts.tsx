...
export function DashboardCharts({ workspaceId }: { workspaceId: string }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setBarData(
        Object.entries(byDay).map(([date, calls]) => ({ date, calls })),
      );
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              {barData.map(({ date, calls }) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      height: `${Math.round((calls / maxCalls) * 160)}px`,
                      minHeight: calls > 0 ? "4px" : "0",
                    }}
                  />
      ...
      <Card>
        ...
        <CardContent>
          {pieData.length === 0 ? (
            <div className="flex h-32 items-center justify-center text-sm text-[#6b6b6b]">
              No call data yet
            </div>
          ) : (
