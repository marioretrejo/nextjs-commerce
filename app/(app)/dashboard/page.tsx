...
async function DashboardContent() {
  ...
  const [
    ...
    supabase
      .from("calls")
      .select("id, duration_seconds, outcome, sentiment, created_at")
      .eq("workspace_id", workspace.id)
  ...
  const dashErr =
    agentsErr ?? activeErr ?? todayErr ?? recentErr ?? campaignsErr;
  ...
  const converted =
    recentCalls?.filter((c) => c.outcome === "converted").length ?? 0;
  const conversionRate =
    totalCalls > 0 ? Math.round((converted / totalCalls) * 100) : 0;
  const avgDuration =
    totalCalls > 0
      ? Math.round(
          (recentCalls?.reduce((s, c) => s + (c.duration_seconds ?? 0), 0) ??
            0) / totalCalls,
        )
      : 0;
    ...
    <Card className="relative overflow-hidden group">
      ...
      <CardContent className="p-5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <p className="text-[10px] text-[#c0c0c0] font-medium shrink-0">
            {sub}
          </p>
        </div>
      </CardContent>
