...
export default function UsagePage() {
  ...
  useEffect(() => {
    fetch("/api/workspace/usage")
      .then((r) =>
        r.ok ? (r.json() as Promise<UsageData>) : Promise.resolve(null),
      )
      .then((d) => {
        setUsage(d);
        setLoading(false);
  ...
  const pct = usage
    ? Math.min((usage.minutes_used / usage.minutes_limit) * 100, 100)
    : 0;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-[#6b6b6b]">
            Your minute consumption and plan billing summary.
          </p>
          ...
          <CardContent className="p-5">
            ...
            {loading ? (
              ...
              <p className="text-3xl font-bold text-[#0a0a0a]">
                ...
                <span className="text-base font-normal text-[#6b6b6b] ml-1">
                  / {usage?.minutes_limit ?? 0} min
                </span>
              </p>
            )}
      ...
      <Card className="mb-6">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            {loading
              ? "Loading…"
              : `${(usage?.minutes_used ?? 0).toFixed(0)} of ${usage?.minutes_limit ?? 0} minutes used`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <>
              ...
              <div className="flex justify-between mt-2 text-xs text-[#6b6b6b]">
                ...
                <span>
                  {Math.max(
                    0,
                    (usage?.minutes_limit ?? 0) - (usage?.minutes_used ?? 0),
                  ).toFixed(0)}{" "}
                  min remaining
                </span>
              </div>
          ...
          {loading ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="h-5 bg-[#f5f5f5] rounded animate-pulse"
                />
              ))}
            </div>
          ) : (
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium capitalize">
                  {usage?.plan ?? "—"} — ${planPrice}/mo
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-[#6b6b6b]">Included minutes</span>
                <span className="font-medium">
                  {(usage?.minutes_limit ?? 0).toLocaleString()} min/mo
                </span>
              </div>
              {(usage?.overage_minutes ?? 0) > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-[#6b6b6b]">
                    Overage ({usage!.overage_minutes} min)
                  </span>
                  <span className="font-medium">
                    ${overageCharge.toFixed(2)}
                  </span>
