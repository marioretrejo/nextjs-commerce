...
import {
  ArrowLeft, Download, Loader2, Pause, Play, Plus, Search, PhoneCall, X,
} from 'lucide-react';
...
interface Campaign {
  ...
  max_concurrency: number; max_retries: number; start_at: string | null;
  workspace_id: string;
...
interface Contact {
  ...
  status: string; attempts: number; last_called_at: string | null;
  call_id: string | null;
...
const STATUS_META: Record<string, { label: string; color: string; bar: string }> = {
  converted:    { label: 'Interested',     color: 'bg-emerald-50 text-emerald-700',  bar: 'bg-emerald-500' },
  rejected:     { label: 'Not Interested', color: 'bg-amber-50 text-amber-700',      bar: 'bg-amber-400'   },
  no_answer:    { label: 'No Answer',      color: 'bg-yellow-50 text-yellow-700',    bar: 'bg-yellow-400'  },
  voicemail:    { label: 'Voicemail',      color: 'bg-slate-100 text-slate-600',     bar: 'bg-slate-400'   },
  invalid:      { label: 'Wrong Number',   color: 'bg-pink-50 text-pink-700',        bar: 'bg-pink-400'    },
  max_attempts: { label: 'Max Attempts',   color: 'bg-red-50 text-red-700',          bar: 'bg-red-500'     },
  calling:      { label: 'In Progress',    color: 'bg-blue-50 text-blue-700',        bar: 'bg-blue-500'    },
  pending:      { label: 'Pending Retry',  color: 'bg-[#f5f5f5] text-[#6b6b6b]',    bar: 'bg-[#d0d0d0]'  },
};

const BAR_ORDER = ['converted','rejected','no_answer','voicemail','invalid','max_attempts','calling','pending'];

// ── Helpers ───────────────────────────────────────────────────────────────────

function exportToCsv(contacts: Contact[], campaignName: string) {
  if (!contacts.length) { toast.error('No contacts to export'); return; }
  const varKeys = Array.from(new Set(contacts.flatMap(c => c.variables ? Object.keys(c.variables) : [])));
  const headers = ['Name', 'Phone', 'Email', 'Status', 'Attempts', 'Last Called', ...varKeys];
  ...
  a.href = url; a.download = `${campaignName}-contacts.csv`; a.click();
...
function TestCallModal({
  ...
  async function start() {
    ...
    try {
      ...
      variableKeys.forEach(k => { if (varVals[k]) variables[k] = varVals[k]; });
      ...
      const d = await res.json() as { call_id?: string; room_name?: string; error?: string };
      if (!res.ok) {
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-sm shadow-2xl mx-auto">
        ...
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-[#e0e0e0]">
          ...
          <button onClick={onClose} className="text-[#6b6b6b] hover:text-[#0a0a0a] rounded p-1">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {callId ? (
          ...
          <div className="px-5 py-8 flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-medium text-[#0a0a0a]">Calling {phone}…</p>
            <p className="text-xs text-[#6b6b6b] font-mono">{callId}</p>
            <Button size="sm" variant="outline" className="mt-2 text-xs" onClick={onClose}>
              Close
            </Button>
          </div>
        ) : (
          ...
          <div className="px-5 py-4 space-y-3">
            ...
            <div className="space-y-1">
              <Label className="text-xs text-[#6b6b6b]">
                International Phone Number <span className="text-red-500">*</span>
              </Label>
              <Input
            ...
            {variableKeys.map(key => (
              <div key={key} className="space-y-1">
                <Label className="text-xs text-[#6b6b6b] capitalize">{key.replace(/_/g, ' ')}</Label>
                <Input
                  placeholder={key}
                  value={varVals[key] ?? ''}
...
function AddContactModal({
  ...
  async function save() {
    ...
    const res = await fetch(`/api/campaigns/${campaignId}/contacts`, {
      ...
      body: JSON.stringify({
        contacts: [{
          name: name || null, phone: phone.trim(), email: email || null,
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-sm shadow-2xl mx-auto">
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-[#6b6b6b]">Phone <span className="text-red-500">*</span></Label>
            <Input placeholder="+1 809 905 2406" value={phone} onChange={e => setPhone(e.target.value)}
              className="h-10 text-sm rounded-xl border-[#e0e0e0] bg-[#f9f9f9]" />
          </div>
          <div className="space-y-1">
          ...
          {variableKeys.map(key => (
            <div key={key} className="space-y-1">
              <Label className="text-xs text-[#6b6b6b] capitalize">{key.replace(/_/g, ' ')}</Label>
              <Input placeholder={key} value={varVals[key] ?? ''} onChange={e => setVar(key, e.target.value)}
                className="h-10 text-sm rounded-xl border-[#e0e0e0] bg-[#f9f9f9]" />
            </div>
          ))}
          <div className="pt-2">
            <Button className="w-full h-11 rounded-xl text-sm" onClick={save} disabled={saving || !phone.trim()}>
...
export default function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function refetchCampaign() {
    ...
    } catch { /* non-fatal background refetch */ }
  }
  ...
  useEffect(() => {
    ...
    return () => { supabase.removeChannel(contactsChannel); supabase.removeChannel(campaignChannel); };
  ...
  const statusCounts = useMemo(() => {
    ...
    BAR_ORDER.forEach(s => { counts[s] = 0; });
    contacts.forEach(c => { counts[c.status] = (counts[c.status] ?? 0) + 1; });
    return counts;
  }, [contacts]);

  const totalInteractions = useMemo(() =>
    contacts.reduce((s, c) => s + c.attempts, 0), [contacts]);

  const connectRate = useMemo(() => {
    const withAttempts = contacts.filter(c => c.attempts > 0).length;
    const connected    = contacts.filter(c => ['converted', 'rejected'].includes(c.status)).length;
    return withAttempts > 0 ? (connected / withAttempts * 100).toFixed(1) : '0.0';
  }, [contacts]);

  const successRate = useMemo(() => {
    const withAttempts = contacts.filter(c => c.attempts > 0).length;
    const converted    = contacts.filter(c => c.status === 'converted').length;
    return withAttempts > 0 ? (converted / withAttempts * 100).toFixed(1) : '0.0';
  }, [contacts]);
  ...
  if (!campaign) return <div className="p-6 text-[#6b6b6b]">Campaign not found.</div>;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-[#0a0a0a]">{campaign.name}</h1>
              <Badge className={
                campaign.status === 'active'  ? 'bg-green-50 border-green-200 text-green-700 text-xs' :
                campaign.status === 'paused'  ? 'bg-amber-50 border-amber-200 text-amber-700 text-xs' :
                campaign.status === 'draft'   ? 'border-[#e0e0e0] text-[#6b6b6b] bg-white text-xs' :
                campaign.status === 'completed' ? 'bg-[#f5f5f5] text-[#0a0a0a] border-[#e0e0e0] text-xs' :
                'border-[#e0e0e0] text-[#6b6b6b] bg-white text-xs'
              }>
                {campaign.status === 'paused' ? 'Stopped' : campaign.status.charAt(0).toUpperCase() + campaign.status.slice(1)}
              </Badge>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {campaign.status === 'draft' || campaign.status === 'paused' ? (
              <Button size="sm" onClick={launch} disabled={acting} className="text-xs">
                {acting ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Play className="w-3.5 h-3.5 mr-1.5" />}
                Launch
              </Button>
            ) : campaign.status === 'active' ? (
              <Button size="sm" variant="secondary" onClick={pause} disabled={acting} className="text-xs">
                <Pause className="w-3.5 h-3.5 mr-1.5" /> Pause
              </Button>
            ) : null}
            {campaign.agent_id && (
              <Button size="sm" variant="outline" className="text-xs" onClick={() => setTestOpen(true)}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 border border-[#e0e0e0] rounded-xl p-5">
            <div className="flex items-baseline justify-between mb-3">
              <p className="text-sm font-medium text-[#0a0a0a]">Total contacts:</p>
              <span className="text-2xl font-bold text-[#0a0a0a]">{campaign.total_contacts.toLocaleString()}</span>
            </div>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-2">
              {BAR_ORDER.filter(s => (statusCounts[s] ?? 0) > 0).map(status => (
                <div key={status} className="flex items-center gap-1.5 text-xs text-[#0a0a0a]">
                  <div className={`w-2 h-2 rounded-full ${STATUS_META[status]?.bar ?? 'bg-[#d0d0d0]'}`} />
                  <span className="text-[#6b6b6b]">{STATUS_META[status]?.label ?? status}:</span>
          ...
          <div className="border border-[#e0e0e0] rounded-xl p-5 space-y-4">
            <div>
              ...
              <p className="text-2xl font-bold text-[#0a0a0a]">{totalInteractions.toLocaleString()}</p>
            </div>
            <div className="grid grid-cols-2 gap-4 pt-2 border-t border-[#e0e0e0]">
              <div>
                <p className="text-xs text-[#6b6b6b]">Connect Rate</p>
                <p className="text-xl font-bold text-[#0a0a0a]">{connectRate}%</p>
                <p className="text-xs text-[#6b6b6b]">{contacts.filter(c => ['converted','rejected'].includes(c.status)).length} contacts</p>
              </div>
              <div>
                <p className="text-xs text-[#6b6b6b]">Success Rate</p>
                <p className="text-xl font-bold text-[#0a0a0a]">{successRate}%</p>
                <p className="text-xs text-[#6b6b6b]">{statusCounts['converted'] ?? 0} contacts</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#e0e0e0] bg-[#fafafa]">
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-[#6b6b6b]">Name and Phone Number</th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-[#6b6b6b]">Attempts</th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-[#6b6b6b]">Last Attempt</th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-[#6b6b6b]">Status</th>
                  <th className="px-4 py-2.5 text-left text-xs font-medium text-[#6b6b6b]">Variables</th>
                </tr>
              </thead>
              <tbody>
                {filteredContacts.slice(0, 200).map(c => {
                  const meta = STATUS_META[c.status];
                  const varEntries = c.variables ? Object.entries(c.variables).slice(0, 3) : [];
                  return (
                    <tr key={c.id} className="border-b border-[#e0e0e0] hover:bg-[#fafafa]">
                      <td className="px-4 py-3">
                        <p className="font-medium text-[#0a0a0a] text-xs">{c.name ?? '—'}</p>
                        <p className="text-xs text-[#6b6b6b] font-mono">{c.phone}</p>
                      </td>
                      <td className="px-4 py-3 text-xs text-[#0a0a0a]">{c.attempts}</td>
                      <td className="px-4 py-3 text-xs text-[#6b6b6b]">
                        {c.last_called_at
                          ? new Date(c.last_called_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
                          : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${meta?.color ?? 'bg-[#f5f5f5] text-[#6b6b6b]'}`}>
                          {meta?.label ?? c.status}
                        </span>
                      </td>
                        ...
                        {varEntries.length > 0
                          ? varEntries.map(([k, v]) => (
                              <span key={k} className="mr-2">
                                <span className="text-[#0a0a0a] font-medium">{k}:</span> {String(v)}
                              </span>
                            ))
                          : '—'}
                      </td>
                ...
                {filteredContacts.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-10 text-center text-xs text-[#6b6b6b]">
