...
export default function CampaignsPage() {
  ...
  useEffect(() => {
    fetch('/api/campaigns')
      .then(r => r.ok ? r.json() as Promise<Campaign[]> : Promise.reject())
  ...
  const totalInteractions = campaigns.reduce((s, c) => s + c.completed_contacts, 0);
  const totalConverted    = campaigns.reduce((s, c) => s + c.converted_contacts, 0);
  const connectRate = totalContacts > 0 ? ((totalInteractions / totalContacts) * 100).toFixed(2) : '0.00';
  const successRate = totalContacts > 0 ? ((totalConverted   / totalContacts) * 100).toFixed(2) : '0.00';
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#e0e0e0] bg-white">
              <th className="px-4 py-3 text-left text-xs font-normal text-[#6b6b6b] w-16">Type</th>
              <th className="px-4 py-3 text-left text-xs font-semibold text-[#0a0a0a]">Campaign name</th>
              <th className="px-4 py-3 text-right text-xs text-[#0a0a0a]">
                <div className="font-semibold">{totalContacts.toLocaleString()}</div>
                <div className="font-normal text-[#6b6b6b]">Total Contacts</div>
              </th>
              <th className="px-4 py-3 text-right text-xs text-[#0a0a0a]">
                <div className="font-semibold">{totalInteractions.toLocaleString()}</div>
                <div className="font-normal text-[#6b6b6b]">Total Interactions</div>
              </th>
              <th className="px-4 py-3 text-right text-xs text-[#0a0a0a]">
              ...
              <th className="px-4 py-3 text-left text-xs font-normal text-[#6b6b6b]">Status</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="px-4 py-14 text-center text-[#6b6b6b] text-sm">
                  {search ? `No campaigns match "${search}"` : 'No campaigns yet'}
                </td>
              </tr>
            ) : (
              filtered.map(campaign => {
                const cr = campaign.total_contacts > 0
                  ? (campaign.completed_contacts / campaign.total_contacts * 100).toFixed(2) : '0.00';
                const sr = campaign.total_contacts > 0
                  ? (campaign.converted_contacts / campaign.total_contacts * 100).toFixed(2) : '0.00';

                return (
                  <tr key={campaign.id} className="border-b border-[#e0e0e0] hover:bg-[#fafafa] transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      ...
                      {campaign.agent && (
                        <p className="text-xs text-[#6b6b6b] mt-0.5">{campaign.agent.name}</p>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <span className={cr !== '0.00' ? 'text-blue-600 font-medium' : 'text-[#6b6b6b]'}>
                        {cr}%{' '}
                        <span className="text-[#6b6b6b] font-normal">({campaign.completed_contacts})</span>
                      </span>
                    </td>
                    <td className="px-4 py-3.5 text-right">
                      <span className={sr !== '0.00' ? 'text-blue-600 font-medium' : 'text-[#6b6b6b]'}>
                        {sr}%{' '}
                        <span className="text-[#6b6b6b] font-normal">({campaign.converted_contacts})</span>
                      </span>
                    </td>
                    <td className="px-4 py-3.5">
