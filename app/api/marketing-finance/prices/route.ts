...
export async function POST(req: Request) {
  ...
  const body = await req.json() as { campaign: string; country: string; price: number; notes?: string };
  if (!body.campaign?.trim()) return NextResponse.json({ error: 'campaign required' }, { status: 400 });
  if (body.price == null || isNaN(body.price)) return NextResponse.json({ error: 'price required' }, { status: 400 });
