...
const AREA_CODE_TZ: Record<string, string> = {
  ...
  '229':'America/New_York','231':'America/New_York','239':'America/New_York','240':'America/New_York',
  ...
  '339':'America/New_York','346':'America/Chicago','347':'America/New_York','351':'America/New_York',
  ...
  '480':'America/Phoenix','484':'America/New_York','501':'America/Chicago','502':'America/New_York',
  ...
  '513':'America/New_York','515':'America/Chicago','516':'America/New_York','517':'America/New_York',
  '518':'America/New_York','520':'America/Phoenix','530':'America/Los_Angeles','531':'America/Chicago',
  '534':'America/Chicago','539':'America/Chicago','540':'America/New_York','541':'America/Los_Angeles',
  ...
  '678':'America/New_York','681':'America/New_York','682':'America/Chicago','701':'America/Chicago',
  ...
  '725':'America/Los_Angeles','727':'America/New_York','731':'America/Chicago','732':'America/New_York',
  ...
  '931':'America/Chicago','936':'America/Chicago','937':'America/New_York','940':'America/Chicago',
  '941':'America/New_York','945':'America/Chicago','947':'America/New_York','949':'America/Los_Angeles',
...
export async function GET(req: Request) {
  ...
  for (const campaign of campaigns as CampaignRow[]) {
    ...
    const { data: dncHits } = await admin
      ...
      .in('phone', phones);
    const dncSet = new Set((dncHits ?? []).map((d: { phone: string }) => d.phone));

    // Mark DNC contacts as rejected so they don't get retried
    const dncContacts = (contacts as ContactRow[]).filter(c => dncSet.has(c.phone));
    if (dncContacts.length) {
      await admin
        .from('campaign_contacts')
        .update({ status: 'rejected' })
        .in('id', dncContacts.map(c => c.id));
    }
