...
export async function POST(req: Request) {
  ...
  if (!body.timezone?.trim()) return NextResponse.json({ error: '"timezone" is required' }, { status: 400 });
  if (!Array.isArray(body.windows)) return NextResponse.json({ error: '"windows" must be an array' }, { status: 400 });
