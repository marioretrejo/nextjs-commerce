...
export async function GET() {
  ...
  const { data, error } = await admin
    .from("sip_trunks")
    .select(
      "id, name, provider, sip_host, port, username, netmask, protocol, region, status, priority, last_tested_at, created_at",
    )
...
export async function POST(req: Request) {
  ...
  const { data, error } = await admin
    ...
    .select(
      "id, name, provider, sip_host, port, username, netmask, protocol, region, status, priority, created_at",
    )
    .single();
