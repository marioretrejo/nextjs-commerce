...
export async function GET() {
  ...
  const { data, error } = await admin
    ...
    .select(
      "id, url, events, description, is_active, last_delivery_at, last_delivery_status, last_delivery_status_code, created_at",
    )
...
export async function POST(req: Request) {
  ...
  if (!url.startsWith("https://") && !url.startsWith("http://localhost")) {
    return NextResponse.json(
      { error: '"url" must use HTTPS (or http://localhost for testing).' },
      { status: 400 },
    );
  }
