...
export async function POST(req: Request) {
  ...
  const { data: phoneRow } = await admin
    .from("phone_numbers")
    .select(
      "id, workspace_id, agent_id, inbound_enabled, routing_rules, status",
    )
  ...
  if (
    !phoneRow ||
    !(phoneRow as { inbound_enabled: boolean }).inbound_enabled
  ) {
  ...
  const phone = phoneRow as {
    ...
    routing_rules: { default_agent_id: string | null };
    status: string;
  };
  ...
  if (!ws || ws.is_suspended)
    return rejectCall("This service is currently unavailable.");
  if (Number(ws.minutes_used) >= Number(ws.minutes_limit)) {
    return rejectCall(
      "We're sorry, this account has reached its usage limit. Please try again later.",
    );
  }
  ...
  try {
    ...
    return rejectCall(
      "We're sorry, we couldn't connect your call right now. Please try again.",
    );
  }
