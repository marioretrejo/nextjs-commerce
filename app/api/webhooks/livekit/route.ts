...
export async function POST(req: Request) {
  ...
  if (event.event === "room_finished") {
    ...
    const match = roomName.match(
      /^(?:agent|sip-agent)-([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})-(\d+)$/i,
    );
  ...
  if (event.event === "room_started") {
    ...
    if (agent) {
      ...
      if (httpUrl && lkApiKey && lkApiSecret) {
        ...
        if (
          ws &&
          Number((ws as { minutes_used: number }).minutes_used) >=
            Number((ws as { minutes_limit: number }).minutes_limit)
        ) {
