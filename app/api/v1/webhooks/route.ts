...
export async function GET(req: Request) {
  ...
  const { data, error } = await admin
    ...
    .select(
      "id, url, events, description, is_active, last_delivery_at, last_delivery_status, last_delivery_status_code, created_at, updated_at",
    )
  ...
  return NextResponse.json(
    {
      ...
      secret,
      _note:
        "Store the secret securely. It will not be shown again. Use it to verify the X-VoiceOS-Signature header.",
    },
    { status: 201 },
  );
}
