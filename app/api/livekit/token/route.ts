...
function sanitizePrompt(raw: string | null | undefined): string | null {
  ...
  return raw
    .replace(
      /ignore\s+(all\s+)?(previous|above|prior|prior\s+to\s+this)\s+(instructions?|prompts?|rules?|context)/gi,
      "[FILTERED]",
    )
    .replace(
      /\b(forget|disregard|override)\s+(everything|all|prior|previous)\b/gi,
      "[FILTERED]",
    )
...
export async function POST(req: Request) {
  ...
  const httpUrl =
    getRegionalHttpUrl(region) || wsUrl.replace("wss://", "https://");
  ...
  const [{ data: agentRow }, { data: workspaceRow }] = await Promise.all([
    ...
    admin
      .from("workspaces")
      .select(
        "id, minutes_used, minutes_limit, plan, active_calls, concurrent_calls_limit, is_suspended",
      )
      .eq("owner_id", user.id)
      .single(),
  ]);
  ...
  const agentWorkspaceId = (agent as unknown as { workspace_id?: string })
    ?.workspace_id;
  ...
  try {
    const queryText = [agent?.name, agent?.system_prompt]
      .filter(Boolean)
      .join(" ")
      .slice(0, 1000);
    ...
    if (queryEmbedding) {
      ...
      if (chunks && (chunks as unknown[]).length > 0) {
        const formatted = (
          chunks as { content: string; source_name: string; kb_name: string }[]
        )
          .map(
            (c, i) =>
              `[${i + 1}] (${c.kb_name} › ${c.source_name})\n${c.content}`,
          )
          .join("\n\n");
  ...
  try {
    ...
    await roomService.createRoom({
      ...
      metadata: JSON.stringify({
        ...
        transfer_number:
          (agent as unknown as Record<string, unknown>)?.["transfer_number"] ??
          null,
        flow_json:
          (agent as unknown as Record<string, unknown>)?.["flow_json"] ?? null,
        flow_config:
          (agent as unknown as Record<string, unknown>)?.["flow_config"] ??
          null,
        ambient_sound:
          (agent as unknown as Record<string, unknown>)?.["ambient_sound"] ??
          null,
    ...
    return NextResponse.json(
      { error: "Call setup failed — please try again." },
      { status: 500 },
    );
  }
