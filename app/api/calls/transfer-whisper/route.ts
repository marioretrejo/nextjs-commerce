...
export async function POST(req: Request) {
  ...
  if (!call.transcript || call.transcript.length < 30) {
    return NextResponse.json({ context: `Transfer from AI agent${call.agent ? ` (${call.agent.name})` : ''}.` });
  }

  const groqKey = process.env['GROQ_API_KEY'];
  if (!groqKey) {
    return NextResponse.json({ context: `Transfer from ${call.agent?.name ?? 'AI agent'} — see call transcript for details.` });
  }
...
Respond with ONLY the one-sentence summary.`;
  ...
  if (!res.ok) {
    return NextResponse.json({ context: `Transfer — ${call.contact_name ?? 'caller'} needs assistance.` });
  }

  const llmData = (await res.json()) as { choices: { message: { content: string } }[] };
  const context = llmData.choices[0]?.message?.content?.trim() ?? `Transfer from ${call.agent?.name ?? 'AI agent'}.`;
