...
export async function POST(req: Request) {
  try {
    ...
    const body = await req.json() as { phone_number?: string; country_code?: string; country_name?: string };
    if (!body.phone_number) return NextResponse.json({ error: 'phone_number is required' }, { status: 400 });
    ...
    if (!buyRes.ok) {
      const errData = await buyRes.json().catch(() => ({})) as { message?: string };
    ...
    const bought = await buyRes.json() as { sid: string; phone_number: string; friendly_name: string };

    const { data: inserted, error: insertErr } = await admin.from('phone_numbers').insert({
      workspace_id: ws.id,
