...
export async function GET() {
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch Twilio numbers — check credentials" },
        { status: 502 },
      );
    }
    ...
    if (twilioNumbers.length > 0) {
      ...
      const toInsert = twilioNumbers.filter(
        (n) => !existingSet.has(n.phone_number),
      );
      ...
      if (toInsert.length > 0) {
        const { error: insertErr } = await admin.from("phone_numbers").insert(
          toInsert.map((n) => ({
            ...
            country_name:
              COUNTRY_NAMES[n.iso_country] ?? n.iso_country ?? "Unknown",
            status: "available",
            twilio_sid: n.sid,
          })),
        );
