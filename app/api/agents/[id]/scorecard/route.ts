...
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const conversionRate =
    totalCalls > 0 ? Math.round((converted / totalCalls) * 100) : 0;
  const scoredCalls = calls.filter((c) => c.qa_score !== null);
  const avgQaScore =
    scoredCalls.length > 0
      ? Math.round(
          scoredCalls.reduce((s, c) => s + (c.qa_score ?? 0), 0) /
            scoredCalls.length,
        )
      : null;
  ...
  const sentimentScore =
    totalCalls > 0 ? Math.round((positiveCount / totalCalls) * 100) : 0;
  ...
  if (!recommendations.length) {
    ...
    if (conversionRate < 20)
      recommendations.push(
        "Your conversion rate is below 20%. Consider reviewing and sharpening the agent's opening message to qualify prospects faster.",
      );
    if (avgDuration > 300)
      recommendations.push(
        "Average call duration exceeds 5 minutes. Tighten the system prompt to help the agent reach its objective more efficiently.",
      );
    if (avgQaScore !== null && avgQaScore < 70)
      recommendations.push(
        "QA score is below 70. Review recent low-scoring calls in the Quality tab and update the system prompt to address common weaknesses.",
      );
    if (sentimentScore < 50)
      recommendations.push(
        "More than half of callers show neutral or negative sentiment. Adjust the agent's personality to be warmer and more empathetic.",
      );
    if (recommendations.length === 0) {
      recommendations.push(
        "Performance looks solid! Keep monitoring QA scores and A/B test new first-message variations to push conversion rates higher.",
      );
    }
  }
