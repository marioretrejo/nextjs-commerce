...
async function scoreCall(
  ...
  if (criteria.length > 0) {
    ...
    scoringSection =
      "Evaluate overall call quality, professionalism, and helpfulness.";
  }
...
export async function POST() {
  ...
  if (!groqKey) {
    return NextResponse.json(
      {
        error:
          "GROQ_API_KEY is not configured. Add it in your environment variables (Render → Settings → Environment Variables).",
  ...
  const eligible = ((calls ?? []) as CallRow[]).filter(
    (c) => c.transcript && c.transcript.length >= 50,
  );
  ...
  if (eligible.length === 0) {
    return NextResponse.json({
      scored: 0,
      message:
        "No eligible calls found (need transcript ≥50 chars and no qa_score yet).",
    });
  }

  // Pre-fetch criteria and system prompts for all unique agents
  const agentIds = [
    ...new Set(eligible.map((c) => c.agent_id).filter(Boolean)),
  ] as string[];
      ...
      agentIds.map((id) =>
        admin
          ...
          .then((r) => ({
            id,
            prompt:
              (r.data as { system_prompt: string | null } | null)
                ?.system_prompt ?? null,
          })),
      ),
    ),
  ...
  const criteriaMap = Object.fromEntries(
    criteriaResults.map((r) => [r.id, r.rows]),
  );
  const promptMap = Object.fromEntries(
    agentResults.map((r) => [r.id, r.prompt]),
  );
  ...
  for (let i = 0; i < eligible.length; i += CONCURRENCY) {
    ...
    await Promise.all(
      batch.map(async (call) => {
        const criteria = call.agent_id
          ? (criteriaMap[call.agent_id] ?? [])
          : [];
        const sysPrompt = call.agent_id
          ? (promptMap[call.agent_id] ?? null)
          : null;
        const hasBasis =
          criteria.length > 0 || (sysPrompt && sysPrompt.length > 20);
