...
export default async function AdminUsersPage() {
  ...
  const { data: users } = await admin
    .from("users")
    .select(
      "id, email, name, plan, minutes_used, minutes_limit, is_suspended, created_at, subscription_status",
    )
    ...
    <div className="min-h-screen bg-[#f5f5f5]">
      <div className="border-b border-[#e0e0e0] bg-white px-6 py-4">
        <div className="flex items-center gap-4">
          ...
          <span className="ml-auto rounded-full bg-[#f5f5f5] px-2.5 py-0.5 text-xs font-medium">
            {userList.length}
          </span>
        </div>
      </div>
        ...
        <div className="rounded-lg border border-[#e0e0e0] bg-white overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-[#f5f5f5] border-b border-[#e0e0e0]">
              <tr>
                {["User", "Plan", "Minutes", "Status", "Joined", ""].map(
                  (h) => (
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-xs font-medium text-[#6b6b6b]"
                    >
                      {h}
                    </th>
                  ),
            ...
            <tbody>
              {userList.map((u) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${u.plan === "scale" || u.plan === "pro" ? "bg-[#0a0a0a] text-white" : "bg-[#f5f5f5] text-[#6b6b6b]"}`}
                    >
                      {u.plan}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${u.is_suspended ? "bg-[#0a0a0a] text-white" : "bg-[#f5f5f5] text-[#0a0a0a]"}`}
                    >
