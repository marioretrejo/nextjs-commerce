...
async function getWhiteLabelBranding(): Promise<WorkspaceBranding | null> {
  try {
    ...
    const appDomain = new URL(process.env['NEXT_PUBLIC_APP_URL'] ?? 'http://localhost:3000').hostname;
    if (!host || host === appDomain || host.includes('localhost') || host.includes('vercel.app')) return null;
    ...
    const { data } = await admin
      ...
      .single();
    return (data as { branding: WorkspaceBranding | null } | null)?.branding ?? null;
  } catch {
    return null;
  }
}
