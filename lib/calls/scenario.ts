...
const DEFAULTS: Record<ScenarioType, Pick<ScenarioHandler, 'action' | 'config' | 'max_attempts'>> = {
  voicemail_short: {
    action:       'hangup',
    config:       {},
    max_attempts: 1,
...
export async function getScenarioHandler(
  ...
  if (data) {
    return { ...(data as Omit<ScenarioHandler, 'isDefault'>), isDefault: false };
  }
 ...
 */
export function classifyVoicemailScenario(durationSeconds: number): ScenarioType {
...
export function buildObjectionInstruction(handler: ScenarioHandler): string {
  const raw = (handler.config['llm_instruction'] as string | undefined) ??
