...
function buildSystemPrompt(agent: Partial<Agent>): string {
  ...
  if (agent.transfer_enabled === true && agent.transfer_number) {
    parts.push(
      `If the customer asks to speak with a human or supervisor, transfer them to ${agent.transfer_number}.`,
    );
  }
...
export async function syncAgentToRetell(
  ...
  const retellAgent = await retellClient.agent.create({
    ...
    voice_emotion: (agent.voice_emotion ?? undefined) as Parameters<
      typeof retellClient.agent.create
    >[0]["voice_emotion"],
    ...
    ambient_sound_volume:
      agent.ambient_sound != null
        ? (agent.ambient_sound_volume ?? 1.0)
        : undefined,
