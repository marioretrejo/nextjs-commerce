...
export async function deliverWebhook(
  ...
  if (event === "call.completed" && payload["room_name"]) {
    const enrichment = await getCallEnrichment(
      admin,
    ...
    enrichedPayload = {
      ...
      duration_seconds:
        enrichment.duration_seconds || payload["duration_seconds"],
    };
  }
  ...
  await Promise.allSettled(
    targets.map(async (target) => {
      const statusCode = await deliverToEndpoint(
        target,
        event,
        enrichedPayload,
      );
      ...
      if (!target.id.startsWith("integration:")) {
        void (async () => {
          ...
          if (rpcErr)
            console.error(
              "[webhooks] record_webhook_delivery failed:",
              rpcErr.message,
            );
        })();
      }
