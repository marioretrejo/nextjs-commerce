...
async function main() {
  ...
  const { data: call } = await admin.from("calls")
    .select("id,status,technical_status,ended_at,answered_at,duration_seconds,end_reason,routing_data")
    .eq("id", CALL_ID).single();
  const rd = call?.routing_data as Record<string,unknown>;
  console.log("call: status=%s technical_status=%s ended_at=%s duration=%ss twilio_sid=%s",
    call?.status, call?.technical_status, call?.ended_at?.slice(11,23) ?? "null",
    call?.duration_seconds, rd?.["twilio_call_sid"] ?? "—");

  const { data: contact } = await admin.from("campaign_contacts")
    .select("status,attempts").eq("id", CONTACT_ID).single();
  console.log("contact: status=%s attempts=%s", contact?.status, contact?.attempts);

  const { data: jobs } = await admin.from("post_call_jobs")
    .select("job_type,status").eq("call_id", CALL_ID);
  console.log("post_call_jobs:", jobs?.length ?? 0, jobs?.map((j:{job_type:string;status:string}) => `${j.job_type}:${j.status}`).join(", ") ?? "none");
  ...
  const { data: zombies } = await admin.from("calls")
    ...
    .eq("workspace_id", WS_ID).is("ended_at", null)
    .order("created_at", { ascending: false }).limit(5);
  console.log("\nZombie calls still open (ended_at=null):", zombies?.length ?? 0);
  (zombies ?? []).forEach((z: {id:string;technical_status:string|null;created_at:string;routing_data:unknown}) => {
    ...
    console.log(`  ${z.id.slice(0,8)} tech=${z.technical_status ?? "null"} created=${z.created_at.slice(11,19)} sid=${sid}`);
  });
}
