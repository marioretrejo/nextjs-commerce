...
async function main() {
  ...
  const { data: call } = await admin
    .from("calls")
    .select(
      "id,technical_status,business_outcome,status,ended_at,answered_at,duration_seconds,end_reason,retell_call_id,routing_data,cost_usd",
    )
    .eq("id", CALL_ID)
    .single();
  ...
  // 3. Call events
  const roomName = (call as { retell_call_id: string | null } | null)
    ?.retell_call_id;
  if (roomName) {
    ...
    (events ?? []).forEach(
      (e: { event_type: string; created_at: string; payload: unknown }) =>
        console.log(
          `  ${e.created_at.slice(11, 23)} ${e.event_type} ${JSON.stringify(e.payload ?? {}).slice(0, 150)}`,
        ),
    );
  } else {
    console.log("\nNo room name — searching events by call_id if available");
    // Try call_id column
    const { data: eventsByCallId } = await admin
      .from("call_events")
      .select("event_type,created_at,payload")
      .eq("call_id", CALL_ID)
      .order("created_at", { ascending: true });
    console.log(`Events by call_id: ${eventsByCallId?.length ?? 0}`);
    (eventsByCallId ?? []).forEach(
      (e: { event_type: string; created_at: string; payload: unknown }) =>
        console.log(
          `  ${e.created_at.slice(11, 23)} ${e.event_type} ${JSON.stringify(e.payload ?? {}).slice(0, 150)}`,
        ),
  ...
  (jobs ?? []).forEach(
    (j: { job_type: string; status: string; error_message: string | null }) =>
      console.log(
