...
async function main() {
  ...
  const { data: call } = await admin
    ...
    .eq("id", "7e929400-964c-40c2-9908-ec9e413ddd64").single();
  console.log("DB call record:", JSON.stringify(call, null, 2));

  const { data: ws } = await admin.from("workspaces").select("active_calls").eq("id", "cd7b409f-82a3-4da2-8f7c-d49c11d62105").single();
