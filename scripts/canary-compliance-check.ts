...
async function main() {
  const { data: camp } = await admin
    .from("campaigns")
    .select("retry_interval_hours,status,name")
    .eq("id", CAMPAIGN_ID)
    .single();
  ...
  console.log(
    `Campaign: "${camp?.name}" status=${camp?.status} retry_interval_hours=${cooldownHours}`,
  );
  ...
  if (recentCall) {
    const minsAgo = Math.round(
      (Date.now() - new Date(recentCall.created_at as string).getTime()) /
        60000,
    );
    console.log(
      `❌ COOLDOWN ACTIVE: call ${recentCall.id} at ${recentCall.created_at} (${minsAgo}min ago)`,
    );
    process.exit(1);
  }
