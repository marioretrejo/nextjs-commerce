...
async function simulateInbound() {
  ...
  console.log(
    "    Check: did the endpoint return TwiML <Dial><Sip>…? (look for text/xml response)",
  );
}
...
async function simulateRoomFinished() {
  const roomName = flag(
    "room",
    `agent-00000000-0000-0000-0000-000000000000-${Date.now()}`,
  );
  ...
  console.log(
    "    Check: Was the call billed? Was deliver.ts called? Did your webhook endpoint receive a POST?",
  );
}
...
async function simulateStatus() {
  ...
  const callSid = flag(
    "sid",
    `CA${crypto.randomBytes(10).toString("hex").toUpperCase()}`,
  );
  ...
  console.log(
    `    Room: ${roomName}  ·  CallSid: ${callSid}  ·  Status: ${status}`,
  );
...
(async () => {
  switch (command) {
    case "inbound":
      ...
      break;
    case "room-finished":
      await simulateRoomFinished();
      break;
    case "status":
      await simulateStatus();
      break;
    default:
      console.log(USAGE);
