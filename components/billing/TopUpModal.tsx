    ...
    >
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Add credit to unlock AI agents, outbound calls, and all platform
            features. You only pay for what you use — no monthly commitment.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-3 gap-2 my-2">
          {[
            ...
            <div
              key={v.text}
              className="rounded-lg bg-[#f9f9f9] border border-[#efefef] p-2.5 text-center"
            >
          ...
          <div className="grid grid-cols-4 gap-2">
            {PRESETS.map((p) => (
              ...
              >
                ...
                <p
                  className={`text-[10px] mt-0.5 ${selected === p.cents ? "text-white/70" : "text-[#a0a0a0]"}`}
                >
                  {p.minutes}
                </p>
              </button>
        ...
        <div className="rounded-xl bg-[#f5f5f5] p-3 space-y-1.5 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-semibold">
              {preset?.label ?? `$${(selected / 100).toFixed(2)}`}
            </span>
          </div>
          <div className="flex justify-between">
        ...
        >
          {loading ? (
            ...
            <>
              <CreditCard className="h-4 w-4 mr-2" /> Pay {preset?.label}{" "}
              Securely
            </>
          )}
        </Button>
