...
export function WaveformPlayer({
  ...
  const activeLine =
    duration > 0 && lines.length > 0
      ? Math.min(
          Math.floor((current / duration) * lines.length),
          lines.length - 1,
        )
      : -1;
  ...
  useEffect(() => {
    ...
    const el = transcriptRef.current.querySelectorAll("[data-line]")[
      activeLine
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      ws.on("error", () => {
        if (!destroyed)
          setError(
            "Could not load audio — the recording URL may have expired.",
          );
      });
    }
      ...
      <div className="rounded-lg border border-[#e0e0e0] bg-[#0a0a0a] p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="font-mono text-sm text-white/70 tabular-nums">
            {fmt(current)} <span className="text-white/30">/</span>{" "}
            {fmt(duration)}
          </div>
        </div>
            ...
            {lines.map((line, i) => {
              ...
              return (
                ...
                >
                  {/* Avatar dot */}
                  <div
                    className={`h-6 w-6 shrink-0 rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5
                  ...
                  <div className={`max-w-[80%] ${isAgent ? "" : "text-right"}`}>
                    <p
                      className={`text-[10px] font-semibold mb-0.5 ${isActive ? "text-amber-600" : "text-[#6b6b6b]"}`}
                    >
                      {speaker}
                      {isActive && playing && (
                    ...
                    <p
                      className={`text-sm leading-relaxed ${isActive ? "text-[#0a0a0a] font-medium" : "text-[#4a4a4a]"}`}
                    >
                      {text}
                    </p>
                  </div>
