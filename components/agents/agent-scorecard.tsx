      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#6b6b6b]">
            Last 30 days · {kpis.totalCalls} calls
          </p>
        </div>
          ...
          <CardContent className="space-y-2">
            {Object.entries(kpis.outcomes)
              ...
              .map(([outcome, count]) => {
                const pct =
                  kpis.totalCalls > 0
                    ? Math.round((count / kpis.totalCalls) * 100)
                    : 0;
                return (
                  <div key={outcome} className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span
                        className={`font-medium px-2 py-0.5 rounded-full ${OUTCOME_COLORS[outcome] ?? "text-[#6b6b6b] bg-[#f5f5f5]"}`}
                      >
      ...
      {recommendations.length > 0 && (
        <Card className="border-[#e0e0e0]">
          <CardHeader>
            ...
            <CardDescription>
              Personalised suggestions based on the last 30 days of call data
            </CardDescription>
          </CardHeader>
