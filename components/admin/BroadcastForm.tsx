      ...
      <CardContent className="space-y-3">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <div className="flex items-center gap-1">
                ...
                <ChevronDown
                  className={`h-3.5 w-3.5 text-[#9b9b9b] transition-transform ${dropdownOpen ? "rotate-180" : ""}`}
                />
              </div>
            </button>
            ...
            {dropdownOpen && (
              <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-[#e0e0e0] rounded-xl shadow-lg overflow-hidden">
                ...
                >
                  <div
                    className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ${sendToAll ? "border-[#0a0a0a] bg-[#0a0a0a]" : "border-[#d0d0d0]"}`}
                  >
                    {sendToAll && (
                ...
                <div className="max-h-48 overflow-y-auto">
                  {filteredUsers.length === 0 ? (
                    <p className="px-3 py-4 text-xs text-[#9b9b9b] text-center">
                      No users found
                    </p>
                  ) : (
                    filteredUsers.map((u) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 ${checked ? "border-[#0a0a0a] bg-[#0a0a0a]" : "border-[#d0d0d0]"}`}
                          >
                            {checked && (
                              <CheckCircle2 className="w-2.5 h-2.5 text-white" />
                            )}
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p
                              className={`text-xs truncate ${u.name ? "text-[#9b9b9b]" : "text-[#0a0a0a] font-medium"}`}
                            >
                              {u.email}
                            </p>
                          </div>
                ...
                {!sendToAll && selectedIds.size > 0 && (
                  <div className="p-2 border-t border-[#f0f0f0]">
                    ...
                    >
                      Confirm {selectedIds.size} recipient
        ...
        {result && (
          ...
          >
            {result.ok ? (
              <>
                <CheckCircle2 className="h-3.5 w-3.5 shrink-0" /> Sent to{" "}
                {result.sent} user{result.sent !== 1 ? "s" : ""} successfully
              </>
            ) : (
              <>
