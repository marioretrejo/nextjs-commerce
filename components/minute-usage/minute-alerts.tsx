    ...
    return (
      <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div className="mx-4 w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
          ...
          <h2 className="mb-2 text-xl font-bold text-[#0a0a0a]">
            You&apos;ve reached your minute limit
          </h2>
          <p className="mb-6 text-sm text-[#6b6b6b]">
            Your plan includes <strong>{limit.toLocaleString()} minutes</strong>{" "}
            per month. You&apos;ve used them all. All outbound calls have been
            stopped. Your minutes reset on <strong>{resetDate}</strong>. Upgrade
            now to continue calling.
          </p>
    ...
    return (
      <div className="rounded-lg border border-orange-300 bg-orange-50 px-4 py-3 text-sm text-orange-900 flex items-center justify-between">
        ...
        <Link href="/billing">
          <Button
            size="sm"
            className="ml-4 shrink-0 bg-orange-600 hover:bg-orange-700 text-white border-transparent"
          >
            Upgrade now
          </Button>
        </Link>
  ...
  if (pct >= 80) {
    return (
      <div className="rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-3 text-sm text-yellow-900 flex items-center justify-between">
        <span>
          <strong>
            ⚠️ You&apos;ve used {minutesUsed.toFixed(0)} of your{" "}
            {limit.toLocaleString()} minutes.
          </strong>{" "}
        ...
        <Link href="/billing">
          <Button
            size="sm"
            variant="outline"
            className="ml-4 shrink-0 border-yellow-400 text-yellow-900 hover:bg-yellow-100"
          >
            Upgrade plan
          </Button>
        </Link>
