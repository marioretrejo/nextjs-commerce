  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      if (perm.state === "denied") {
        ...
        try {
          ...
          warnings.push(
            "Microphone access was denied. The call cannot proceed without it.",
          );
        }
      }
    } catch {
    ...
    try {
      ...
      if (rttMs > maxRttMs) {
        ...
        warnings.push(
          `Moderate network latency detected (${rttMs}ms). Call should still work fine.`,
        );
      }
    } catch {
    ...
    if (micPermission === "denied" || micPermission === "error") {
      ...
      estimatedBandwidthKbps < minBandwidthKbps
    ) {
      finalStatus = "poor";
    }
